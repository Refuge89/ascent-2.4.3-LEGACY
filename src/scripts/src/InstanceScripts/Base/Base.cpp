...
EmoteDesc* MoonScriptCreatureAI::AddEmote(EventType pEventType, const char* pText, TextType pType, uint32 pSoundId)
{
	...
	if( pText || pSoundId )
	{
		...
		switch( pEventType )
		{
			...
			case Event_OnCombatStop:    mOnCombatStopEmotes.push_back(NewEmote); break;
...
void MoonScriptCreatureAI::RemoveEmote(EventType pEventType, EmoteDesc* pEmote)
{
	switch( pEventType )
	{
        ...
        case Event_OnCombatStop:    mOnCombatStopEmotes.push_back(pEmote); break;
...
void MoonScriptCreatureAI::RemoveAllEmotes(EventType pEventType)
{
	switch( pEventType )
	{
        ...
        case Event_OnCombatStop:    DeleteArray(mOnCombatStopEmotes);
...
void MoonScriptCreatureAI::SetWaveUnitPreferences()
{
	_unit->GetAIInterface()->setMoveType(MOVEMENTTYPE_FORWARDTHANSTOP);
	//_unit->GetAIInterface()->m_moveBackward = true;	// this can make mobs move crazy! (if it's false (default) it may force mob to go to wp 1; if it's true to last one o_O
	SetSummonPreferences(false, 0);
}

void MoonScriptCreatureAI::SetSummonPreferences(bool Despawn, uint32 Delay)
{
	...
	if (Despawn)
		MoonScriptCreatureAI::Despawn(Delay, 0);
}

void MoonScriptCreatureAI::SetSide(uint32 pSide)
{
	mSide = pSide;
}

void MoonScriptCreatureAI::SetPosition(uint32 pPosition)
{
	mPosition = pPosition;
}
