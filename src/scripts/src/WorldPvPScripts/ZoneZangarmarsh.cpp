...
public:
	...
	void AIUpdate()
	{
		...
		PlayerSet::iterator itr = _gameobject->GetInRangePlayerSetBegin();		
		PlayerSet::iterator itrend = _gameobject->GetInRangePlayerSetEnd();
...
public:
    ...
    void GossipSelectOption(Object* pObject, Player*  plr, uint32 Id, uint32 IntId, const char * Code)
    {
		...
		pCreature = pObject->IsCreature() ? static_cast<Creature*>( pObject ) : NULL;
		if( !pCreature )
			return;
...
void ZMSpawnBanners(MapMgr* bmgr, int32 side)
{
	...
	bGo->SetUInt32Value(GAMEOBJECT_STATE, b->state);
...
void ZMSpawnObjects(MapMgr* pmgr)
{
	...
	for(i = 0; i < 2; ++i)
	{
		...
		pGo->SetUInt32Value(GAMEOBJECT_STATE, p->state);
