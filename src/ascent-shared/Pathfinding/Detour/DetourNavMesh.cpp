					 ...
					 int maxTiles, int maxPolys, int maxNodes)
{
	...
	m_posLookup.resize(m_tileLutSize, NULL);
	...
	m_nodePool = std::auto_ptr<dtNodePool>(new dtNodePool(maxNodes, nextPow2(maxNodes/4)));
	m_openList = std::auto_ptr<dtNodeQueue>(new dtNodeQueue(maxNodes));
...
bool dtNavMesh::init(std::vector<unsigned char> & data, int dataSize, bool ownsData, int maxNodes)
{
	// Make sure the data is in right format.
	dtMeshHeader* header = (dtMeshHeader*)&data[0];
...
bool dtNavMesh::addTileAt(int x, int y, std::vector<unsigned char> & data, int dataSize, bool ownsData)
{
	if (getTileAt(x, y))
		return false;
	...
	// Init tile.
	tile->data = data;
	tile->header = (dtMeshHeader*)&tile->data[0];
	tile->x = x;
	tile->y = y;
	...
	const int vertsSize = align4(sizeof(float)*3*tile->header->vertCount);
	const int polysSize = align4(sizeof(dtPoly)*tile->header->polyCount);
	const int linksSize = align4(sizeof(dtLink)*(tile->header->maxLinkCount));
	const int detailMeshesSize = align4(sizeof(dtPolyDetail)*tile->header->detailMeshCount);
	const int detailVertsSize = align4(sizeof(float)*3*tile->header->detailVertCount);
	const int detailTrisSize = align4(sizeof(unsigned char)*4*tile->header->detailTriCount);
	const int bvtreeSize = align4(sizeof(dtBVNode)*tile->header->bvNodeCount);
	const int offMeshLinksSize = align4(sizeof(dtOffMeshConnection)*tile->header->offMeshConCount);
	
	unsigned char* d = &tile->data[headerSize];
	...
	tile->links[tile->header->maxLinkCount-1].next = DT_NULL_LINK;
	for (int i = 0; i < tile->header->maxLinkCount-1; ++i)
	{
...
dtPolyRef dtNavMesh::getTileId(const dtMeshTile* tile) const
{
	...
	unsigned int it = tile - &m_tiles[0];
...
bool dtNavMesh::closestPointOnPoly(dtPolyRef ref, const float* pos, float* closest) const
{
	...
	if (ip >= (unsigned int)header->polyCount) 
		return false;
						...
						dtPolyRef* path, const int maxPathSize)
{
	...
	if (!m_nodePool.get() || !m_openList.get())
		return 0;
									...
									const int maxResult)
{
	...
	if (!m_nodePool.get() || !m_openList.get()) return 0;
