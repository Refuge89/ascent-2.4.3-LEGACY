...
struct dtMeshTile
{
	...
	std::vector<unsigned char> data;		// Pointer to tile data.
...
public:
	...
	// *) Will be rounded to next power of two.
	// Returns: True if succeed, else false.
	bool init(std::vector<unsigned char> & data, int dataSize, bool ownsData, int maxNodes);
	...
	bool addTileAt(int x, int y, std::vector<unsigned char> & data, int dataSize, bool ownsData);
	...
	dtMeshTile* __fastcall getTileAt(int x, int y);
	...
	bool __fastcall closestPointOnPoly(dtPolyRef ref, const float* pos, float* closest) const;
...
private:
	...
	bool __fastcall getEdgeMidPoint(dtPolyRef from, dtPolyRef to, float* mid) const;
	bool __fastcall getEdgeMidPoint(dtPolyRef from, const dtPoly* fromPoly, const dtMeshTile* fromTile,
									dtPolyRef to, const dtPoly* toPoly, const dtMeshTile* toTile,
									float* mid) const;
	...
	std::vector<dtMeshTile*> m_posLookup;		// Tile hash lookup.
	dtMeshTile* m_nextFree;						// Freelist of tiles.
	std::vector<dtMeshTile> m_tiles;			// List of tiles.
	...
	std::auto_ptr<class dtNodePool>  m_nodePool;		// Pointer to node pool.
	std::auto_ptr<class dtNodeQueue> m_openList;		// Pointer to open list queue.
};
