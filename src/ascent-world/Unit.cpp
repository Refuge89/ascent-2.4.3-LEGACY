...
{
    enum Spells //Didn't want to conflict with another Spells enumaration, couldn't think of another name for this though
	{
		...
		for(itx = m_extraStrikeTargets.begin(); itx != m_extraStrikeTargets.end();)
		{
			...
			for(set<Object*>::iterator itr = m_objectsInRange.begin(); itr != m_objectsInRange.end(); ++itr)
			{
				...
				if(CalcDistance(*itr) < 5.0f && isAttackable(this, (*itr)) && (*itr)->isInFront(this) && !TO_UNIT(*itr)->IsPacified())
				{
					...
					bool skip_hit_check2 = ex->spell_info->Id == SPELL_SWEEPING_STRIKES ? true : false;
					Strike(TO_UNIT(pVictim), weapon_damage_type, ex->spell_info, add_damage, pct_dmg_mod, exclusive_damage, false, skip_hit_check2);
					break;
				}
			}

			// Sweeing strikes stacks should be removed regardless of the situation, if they are not fully consumed by attacking a target
			if(ex->charges > 0)
			{
				ex->charges--;
...
bool Unit::HasOnDamageInteruptAura() const
{
	for(uint32 x = MAX_POSITIVE_AURAS;x < MAX_AURAS;++x)
	{
		if(m_auras[x])
		{
			if(m_auras[x]->GetSpellProto()->AuraInterruptFlags & AURA_INTERRUPT_ON_ANY_DAMAGE_TAKEN)
				return true;
		}
	}
	return false;
...
inline void Unit::DoCast(Unit* Target, uint32 SpellID, bool triggered)
{
...
bool Unit::CanSee(Object * obj)
{
	if(IsCreature())
	{
		if(obj->IsUnit())
			return TO_CREATURE(this)->CanSee(TO_UNIT(obj));
		else
			return false;
	}
	else if(IsPlayer())
	{
		return TO_PLAYER(this)->CanSee(obj);
	}
	else
		return false;
}
