...
bool Player::Create(WorldPacket& data )
{
   ...
   // SetUInt32Value(PLAYER_BYTES_2, (facialHair | (0xEE << 8) | (0x01 << 16) | (0x02 << 24)));
	...
	SetUInt32Value(PLAYER_LEVEL_CAP, sWorld.m_levelCap);
...
void Player::SaveToDB(bool bNewCharacter /* =false */)
{
	...
	<< m_uint32Values[PLAYER_FIELD_KNOWN_TITLES] << ","
...
#define get_next_field fields[field_index++]
	...
	SetUInt32Value(PLAYER_FIELD_KNOWN_TITLES, PvPRanks[GetPVPRank() + offset]);
...
void Player::OnPushToWorld()
{
	...
	*data << uint32(0);
	...
	// send world states
	//m_mapMgr->GetWorldStateManager()->SendWorldStates(this);
...
void Player::DuelCountdown()
{
	...
	if( m_duelCountdownTimer == 0 )
	{
		...
		//Bfx Health +mana And Cooldown Reset in Durotar And Elwynn Forest.
...
void Player::save_PVP()
{
	CharacterDatabase.Execute("UPDATE characters SET pvprank = %u, selected_pvp_title = %u, available_pvp_title = %u WHERE guid = %u",
		uint32(GetPVPRank()), m_uint32Values[PLAYER_CHOSEN_TITLE], m_uint32Values[PLAYER_FIELD_KNOWN_TITLES], m_uint32Values[OBJECT_FIELD_GUID]);
}
