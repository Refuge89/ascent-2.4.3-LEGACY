...
#define SPDT_SAMPLINGRATE				2000	// there is no point flooding calculations	
#define SPDT_DETECTION_ERROR			0.70f	// ex: 10% for a speed=7 => 0.7 max speed cheat
#define CHEAT_ALARMS_TO_TRIGGER_CHEAT	6		// if x alarms stack up over time then it is time to kick the player
...
class SpeedCheatDetector
{
	public:
		SpeedCheatDetector();
		void			AddSample(float x, float y, uint32 stamp, float player_speed, uint32 latency); // update the detector with new values
		ARCEMU_INLINE	char	IsCheatDetected(){ return cheat_threat>= CHEAT_ALARMS_TO_TRIGGER_CHEAT; } // test cheater status
		void			SkipSamplingUntil(uint32 stamp);	// delay then reset cheat detector
		void			ReportCheater(Player *_player);	// take actions agains a cheater
		void			EventSpeedChange(float player_speed);				// reset internal values on speed change
