...
public:
  ...
  //void StrikeWithAbility( Unit* pVictim, Spell* spell, uint32 addspelldmg, uint32 weapon_damage_type );
    ...
    Unit* GetVictim() const { return m_attacking; } // returns current unit thats attacking a creature
...
//	void HandleProcSpellOnSpell(Unit* Victim,uint32 damage,bool critical);//nasty, some spells proc other spells
	...
	inline void DoCast(Unit* Target, uint32 SpellID, bool triggered);
    ...
    //Handlers for extra strikes from abilities such as sweeping strikes which hits more then one target
	...
	// Stealth  
	ASCENT_INLINE int32 GetStealthLevel() { return m_stealthLevel; }void RemoveAllMovementImpairing();
	...
	bool HasOnDamageInteruptAura() const;
...
//	std::map<uint32,ProcTriggerSpellOnSpellList> m_procSpellonSpell; //index is namehash
	...
	bool IsNeutralFaction()
	{
		if (m_factionDBC &&
			m_factionDBC->RepListId == -1 &&
			m_faction->HostileMask == 0 &&
			m_faction->FriendlyMask == 0)
		{
			return true;
		}
	...
	bool CanSee(Object * obj); // Bfx Los New Functions
