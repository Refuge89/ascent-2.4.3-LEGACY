...
// Quick Check to see if target Can attack
bool AIInterface::UnsafeCanOwnerAttackUnit(Unit *pUnit)
{
	...
	//don't agro neutrals
	if((pUnit->IsPlayer() || pUnit->IsPet()) && m_Unit->IsNeutralFaction())
		return false;
	else if((m_Unit->IsPlayer() || m_Unit->IsPet()) && pUnit->IsNeutralFaction())
		return false;
	...
	if(!isAttackable(m_Unit, pUnit, !m_Unit->CanSee(pUnit)))
		return false;

	// we stop if the target pet wearing an aura that break attack
	// Thus for example would be released from the trap target
	if(m_Unit->IsPet() && pUnit->HasOnDamageInteruptAura())
		return false;
