...
void AVNode::Spawn()
{
	...
	else
	{
		...
		if( m_flag == NULL )
		{
			...
			m_flag->SetUInt32Value(GAMEOBJECT_ANIMPROGRESS, 100);
			m_flag->SetUInt32Value(GAMEOBJECT_DYN_FLAGS, 1);
		...
		else
		{
			...
			if( m_flag->GetEntry() != g->id[m_state] || !m_flag->IsInWorld() )
			{
				...
				m_flag->SetUInt32Value(GAMEOBJECT_TYPE_ID, goi->Type);
				m_flag->SetUInt32Value(GAMEOBJECT_FACTION, g_gameObjectFactions[m_state]);
				m_flag->SetUInt32Value(GAMEOBJECT_ANIMPROGRESS, 100);
				m_flag->SetUInt32Value(GAMEOBJECT_DYN_FLAGS, 1);
				m_flag->PushToWorld(m_bg.GetMapMgr());
	...
	else
	{
		...
		if( m_aura == NULL )
		{
			...
			m_aura->SetUInt32Value(GAMEOBJECT_ANIMPROGRESS, 100);
			m_aura->SetUInt32Value(GAMEOBJECT_FLAGS, GO_FLAG_IN_USE);
			m_aura->SetUInt32Value(GAMEOBJECT_STATE, 1);
		...
		else
		{
			...
			if( m_aura->GetEntry() != g->id[m_state] || !m_aura->IsInWorld() )
			{
				...
				m_aura->SetUInt32Value(GAMEOBJECT_TYPE_ID, goi->Type);
				m_aura->SetUInt32Value(GAMEOBJECT_FACTION, g_gameObjectFactions[m_state]);
				m_aura->SetUInt32Value(GAMEOBJECT_ANIMPROGRESS, 100);
				m_aura->SetUInt32Value(GAMEOBJECT_FLAGS, GO_FLAG_IN_USE);
				m_aura->SetUInt32Value(GAMEOBJECT_STATE, 1);
	...
	else
	{
		...
		if( m_glow == NULL )
		{
			...
			m_glow->SetUInt32Value(GAMEOBJECT_ANIMPROGRESS, 100);
			m_glow->SetUInt32Value(GAMEOBJECT_FLAGS, GO_FLAG_IN_USE);
			m_glow->SetUInt32Value(GAMEOBJECT_STATE, 1);
		...
		else
		{
			...
			if( m_glow->GetEntry() != g->id[m_state] || !m_glow->IsInWorld() )
			{
				...
				m_glow->SetUInt32Value(GAMEOBJECT_TYPE_ID, goi->Type);
				m_glow->SetUInt32Value(GAMEOBJECT_FACTION, g_gameObjectFactions[m_state]);
				m_glow->SetUInt32Value(GAMEOBJECT_ANIMPROGRESS, 100);
				m_glow->SetUInt32Value(GAMEOBJECT_FLAGS, GO_FLAG_IN_USE);
				m_glow->SetUInt32Value(GAMEOBJECT_STATE, 1);
	...
	if( m_template->m_worldStateFields[m_lastState] != 0 )
		m_bg.GetMapMgr()->GetStateManager().UpdateWorldState(m_template->m_worldStateFields[m_lastState], 0);
	...
	if( !m_template->m_capturable )
	{
		if( m_state != m_template->m_defaultState )
		{
			...
			if( m_flag != NULL )
			{
				...
				m_flag->SetUInt32Value(GAMEOBJECT_STATE, 1);
			}
...
void AlteracValley::OnCreate()
{
	...
	gate->SetFloatValue(GAMEOBJECT_ROTATION_02, 0.36f);
	gate->SetFloatValue(GAMEOBJECT_ROTATION_03, 0.922766f);
	gate->PushToWorld(m_mapMgr);
	m_gates.push_back(gate);
